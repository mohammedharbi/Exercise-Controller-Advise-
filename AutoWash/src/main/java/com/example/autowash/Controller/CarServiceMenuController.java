...
public class CarServiceMenuController {
    ...
    @PostMapping("/add")
    public ResponseEntity addCarServiceMenu(@RequestBody @Valid CarServiceMenu carServiceMenu){
        carServicesMenuService.addCarServiceMenu(carServiceMenu);
        return ResponseEntity.status(201).body("Car service added");
    }

    @PutMapping("/update/{id}")
    public ResponseEntity updateCarServiceMenu(@PathVariable Integer id, @RequestBody @Valid CarServiceMenu carServiceMenu){
