...
public class CarServicesMenuService {
    ...
    public List<CarServiceMenu> getServiceByAutoWashCar(Integer autoWashCarId){
        List<CarServiceMenu> getServicesByAutoWashCar = carServiceMenuRepository.getCarServiceMenuByAutoWashCarId(autoWashCarId);
        if (getServicesByAutoWashCar.isEmpty()){throw new ApiException("No services found for the auto wash car");}
        return getServicesByAutoWashCar;
    }
    ...
    public CarServiceMenu getCheapestPrice(Integer userId) {
        ...
        if (tempCarServiceMenu == null){throw new ApiException("no service found to compare for the cheapest price");}
